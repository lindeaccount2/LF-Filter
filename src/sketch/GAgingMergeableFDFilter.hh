#ifndef SKETCH_GAGINGMERGEABLEFDFILTER_HH
#define SKETCH_GAGINGMERGEABLEFDFILTER_HH
...
template <typename hash_t>
GAgingMergeableFDFilter<hash_t>::GAgingMergeableFDFilter(int k, int kk, int nbits, int num_hash,
									int gnbits, int gnum_hash, uint64_t delay_thres, int block_size, int period)
...
{
	// std::cout << "In GAgingMergeableFDFilter:" << k << " " << kk << endl;
...
template <typename hash_t> GAgingMergeableFDFilter<hash_t>::~GAgingMergeableFDFilter() {}

template <typename hash_t>
template <int32_t key_len>
uint64_t GAgingMergeableFDFilter<hash_t>::update(const FlowKey<key_len> &flowkey, uint64_t timestamp) {
...
template <int32_t key_len>
uint64_t
GAgingMergeableFDFilter<hash_t>::estimateTimeByOldBfs(const FlowKey<key_len> &flowkey) {
  uint64_t interval = 0;
...
template <typename hash_t>
void GAgingMergeableFDFilter<hash_t>::mergeOldBfs(const BitBf<hash_t> &bf) {
...
template <typename hash_t> size_t GAgingMergeableFDFilter<hash_t>::size() const {
	std::cout << "GAgingMergeableFDFilter size: " << gbf_.size() << " " << empty_bf_.size() << " " <<  bfs_.size() << " " <<  old_bfs_.size() << " " << kk_ << std::endl;
  return gbf_.size() + empty_bf_.size() * bfs_.size() + empty_bf_.size() * old_bfs_.size() / kk_;
}

template <typename hash_t> 
auto GAgingMergeableFDFilter<hash_t>::clear() -> void {
  for(auto &bf : bfs_){
    bf.clear();
  }
}
