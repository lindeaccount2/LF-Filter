...
template <typename hash_t>
BloomFilter<hash_t> &
BloomFilter<hash_t>::operator=(BloomFilter<hash_t> bf) noexcept {   // 为什么是swap？
  bf.swap(*this);
  return *this;
}
...
template <typename hash_t>
int BloomFilter<hash_t>::getNbitsBySize(int num_hash, int mem_size) {         // 这个的计算是？
  int nbits =
...
template <typename hash_t>
void BloomFilter<hash_t>::And(const BloomFilter<hash_t> &rhs) {   // assert 都做一遍太费时间
