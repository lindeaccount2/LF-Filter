...
private:
    ...
    // --- 统计变量 (补充了 cost_predictor_) ---
    long long total_heavy_queries_ = 0;
    long long pred_hits_ = 0;
    long long skipped_lookups_ = 0; 
    long long cost_baseline_ = 0;   
    long long cost_predictor_ = 0; // [新增] 用于统计预测器实际开销

public:
    LFFilter(int n_h, int n_w, int n_c, int k_w, int slots, int /*d1*/, int /*d2*/, uint64_t delay_thres, int heavy_thres)
    ...
    uint64_t update(const FlowKey<key_len> &flowkey, uint64_t timestamp) {
        ...
        size_t h = hash_fn_(flowkey); // 哈希复用
...
private:
    // --- [修改] query_internal: 增加查表法统计 Cost ---
    template <int32_t key_len>
    uint64_t query_internal(const FlowKey<key_len> &flowkey, uint64_t timestamp, size_t h) {
        ...
        if (found != -1) {
            ...
            // 3. [新增] 使用查表法统计 Predictor Cost (最小化性能开销)
            // 矩阵: [predicted][found]
            static const int cost_matrix[3][3] = {
                {1, 2, 3}, // Pred=0: Hit0=1, Hit1=2, Hit2=3
                {2, 1, 3}, // Pred=1: Hit0=2, Hit1=1, Hit2=3
                {3, 2, 1}  // Pred=2: Hit0=3, Hit1=2, Hit2=1
            };
