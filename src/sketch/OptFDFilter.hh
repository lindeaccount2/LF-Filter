#ifndef SKETCH_OPTFDFILTER_HH
#define SKETCH_OPTFDFILTER_HH
...
public:
  OptFDFilter(int k, int kk, int ncounters, int num_hash, int gnbits, int gnum_hash, uint64_t delay_thres);
  ...
  std::string name() { return "OptFDFilter"; };
};

template <typename hash_t>
OptFDFilter<hash_t>::OptFDFilter(int k, int kk, int ncounters, int num_hash, 
...
template <typename hash_t> 
OptFDFilter<hash_t>::~OptFDFilter() {}

template <typename hash_t>
template <int32_t key_len>
void OptFDFilter<hash_t>::insertBloomFilter(int bf_index, const FlowKey<key_len> &flowkey, int value) {
...
template <typename hash_t>
template <int32_t key_len>
int OptFDFilter<hash_t>::queryKey(const FlowKey<key_len> &flowkey) {
	auto start_point = std::chrono::steady_clock::now();
...
template <typename hash_t>
bool OptFDFilter<hash_t>::checkGroup(int group_id) {
	int block_id = 0;
	int block_offset = 0;
	uint16_t base = 0xffff;
...
template <typename hash_t>
void OptFDFilter<hash_t>::clearBloomFilter(int bf_index) {
	int group_id = bf_index >> 3;
	int group_offset = bf_index - (group_id << 3);
	uint64_t base = ((1<<kk_) - 1) << ((7-group_offset)*kk_);
	base |= (base << 16);
	base |= (base << 32);
	base = ~(base);
...
template <typename hash_t>
template <int32_t key_len>
uint64_t OptFDFilter<hash_t>::update(const FlowKey<key_len> &flowkey, uint64_t timestamp) {
	// 下一个时间窗口
	++thput_num_;
	auto start_point = std::chrono::steady_clock::now();
...
template <typename hash_t> 
size_t OptFDFilter<hash_t>::size() const {
	return sizeof(bfs_[0]) * block_num * group_num_per_block + gbf_.size();
}

template <typename hash_t> 
auto OptFDFilter<hash_t>::clear() -> void {
